...
int main(int argc, char * argv[]) {
  ...
  //! [part4]
  vector<string> bkg_procs = {"ZJ","ZTT_ml","ZTT_mt", "W", "QCD", "TT","VV"};
  cb.AddProcesses({"*"}, {"MuTauFR"}, {"13TeV"}, {"mt"}, bkg_procs, cats, false);

  vector<string> sig_procs = {"ZMM"};
  ...
  //! [part5]
    cb.cp().process(ch::JoinStr({sig_procs,{"ZJ"},{"ZTT_ml"},{"ZTT_mt"},{"VV"},{"TT"}})).AddSyst(cb, "lumi_$ERA", "lnN", SystMap<era>::init({"13TeV"}, 1.026));
    
  //! [part5]

  //! [part6]
    //cb.cp().process(ch::JoinStr({sig_procs})).AddSyst(cb, "tagmuon_", "shape", SystMap<>::init(1));
    cb.cp().process(ch::JoinStr({sig_procs})).AddSyst(cb, "probemuon_", "shape", SystMap<>::init(1));
    ...
    //cb.cp().process(ch::JoinStr({sig_procs})).AddSyst(cb, "normalizationDYMuMu", "lnN", SystMap<>::init(1.06));
  ...
  //! [part9]
    ...
    << cb.cp().bin({"MuTauFR_pass"}).process({"ZMM"}).GetRate()/((cb.cp().bin({"MuTauFR_pass"}).process({"ZMM"}).GetRate()+cb.cp().bin({"MuTauFR_fail"}).process({"ZMM"}).GetRate())) << "\n";
